				...
				max_omega_(0.5)
{
	...
	joy_sub_ = nh_.subscribe<sensor_msgs::Joy>("joy", 10, &DiyazenTeleop::joyCallback, this);	
	goal_sub_ = nh_.subscribe<geometry_msgs::PoseStamped>("move_base/current_goal", 10, &DiyazenTeleop::goalCallback, this); 

	timer_ = nh_.createTimer(ros::Duration(0.1), boost::bind(&DiyazenTeleop::publish, this));
	
}

void DiyazenTeleop::goalCallback(const geometry_msgs::PoseStamped::ConstPtr& goal){
...
void DiyazenTeleop::joyCallback(const sensor_msgs::Joy::ConstPtr& joy){
...
bool DiyazenTeleop::IsZeroMsg(geometry_msgs::Twist cmd_vel)
{
  bool result = true;
...
{
	ros::init(argc, argv, "diyazen_teleop");
	DiyazenTeleop diyazen_teleop;
